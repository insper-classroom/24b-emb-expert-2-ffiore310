...
void task_bme280(void *pvParameters) {
    printf("Initializing BME280 via SPI...\n");
...
void display_task(void *p) {
    ...
    while (1) {
        if (xQueueReceive(xQueue, &sensor_data, portMAX_DELAY) == pdPASS) {
            GFX_clearScreen();
            GFX_setCursor(0, 0);
            GFX_printf("Temp: %.2f C\n", sensor_data.temperature);
            GFX_printf("Pressure: %.2f Pa\n", sensor_data.pressure);
...
int main()
{
    ...
    xQueue = xQueueCreate(32, sizeof(SensorData));

    xTaskCreate(display_task,"display_task", 4096, NULL, 1, NULL);
    xTaskCreate(task_bme280, "bme280_Task", 2048, NULL, 1, NULL);
